...
class PricePredictionForm(forms.Form):
    bedroom = forms.IntegerField(label='Bedrooms', min_value=0)
    bathroom = forms.IntegerField(label='Bathrooms',min_value=0)
    sqft_living = forms.IntegerField(label='Sqft Living',min_value=0)
    sqft_lot = forms.IntegerField(label='Sqft Lot',min_value=0)
    floors = forms.DecimalField(label='Floors',min_value=0,decimal_places=2)
